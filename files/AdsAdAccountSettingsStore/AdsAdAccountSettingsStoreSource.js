__d(
  "AdsAdAccountSettingsStoreSource",
  [
    "AdsAdAccountSettingsStoreSourceServerQuery.graphql",
    "AdsVideoOptimizationVariants",
    "RelayHooks",
    "ReverseInteropStoreSourceWeakBase",
    "promiseDone",
    "readQueryToLoadObject",
  ],
  function (t, n, r, o, a, i, l) {
    "use strict";
    var e,
      s =
        e !== void 0
          ? e
          : (e = n("AdsAdAccountSettingsStoreSourceServerQuery.graphql")),
      u = (function (e) {
        function t() {
          var t = "ads_ad_account_settings_from_ad_account";
          return e.call(this, t) || this;
        }
        babelHelpers.inheritsLoose(t, e);
        var n = t.prototype;
        return (
          (n.__getQuery = function () {
            return s;
          }),
          (n.__getVariables = function () {
            return { ad_account_id: "" };
          }),
          (n.__getGQLParams = function (t) {
            return { ad_account_id: t };
          }),
          (n.__fetchQuery = function (t, n) {
            var e = this,
              a = "ads_ad_account_settings_from_ad_account",
              i = "AdsAdAccountSettings",
              l = t[0],
              u = this.getRelayId(l);
            this.updateRecords([u], a, i);
            var c = this.__getGQLParams(l),
              d = o("RelayHooks")
                .fetchQuery(this.__environment, s, c)
                .toPromise()
                .then(function (t) {
                  return e.__toReturnType(t);
                })
                .catch(function (r) {
                  return e.handleFetchQueryError(t, n, r);
                });
            r("promiseDone")(d, function () {
              return e.notifyDataChange();
            });
          }),
          (n.__getCached = function (t, n) {
            var e = this,
              o = t.map(function (t) {
                var n = r("readQueryToLoadObject")(
                  e.__environment,
                  s,
                  e.__getGQLParams(t),
                  function (t) {
                    return e.__toReturnType(t);
                  },
                );
                return { key: t, value: n };
              });
            return o;
          }),
          (n.__toReturnType = function (t) {
            var e,
              n,
              r,
              o,
              a =
                (e =
                  t == null ||
                  (n = t.ads_ad_account_settings_from_ad_account) == null
                    ? void 0
                    : n.feature_sticky_entries) != null
                  ? e
                  : [],
              i = {};
            for (var l of a)
              if (l != null && l.feature_name != null) {
                var s;
                i[l.feature_name] = {
                  status: m(l.status),
                  timestamp: (s = l.timestamp) != null ? s : 0,
                };
              }
            var u =
                (r =
                  t == null ||
                  (o = t.ads_ad_account_settings_from_ad_account) == null
                    ? void 0
                    : o.video_optimization_sticky_entries) != null
                  ? r
                  : [],
              p = {};
            for (var _ of u) {
              var f = c(_ == null ? void 0 : _.optimization);
              if (_ != null && f != null) {
                var g;
                p[f] = {
                  status: d(_.status),
                  timestamp: (g = _.timestamp) != null ? g : 0,
                };
              }
            }
            return {
              aplusc_sticky_config: i,
              media_optimization_sticky_config: p,
            };
          }),
          t
        );
      })(r("ReverseInteropStoreSourceWeakBase"));
    function c(e) {
      if (e == null) return null;
      switch (e) {
        case "AI_VIDEO_SHORTENING":
          return o("AdsVideoOptimizationVariants").AI_VIDEO_SHORTENING;
        case "AUTO_VIDEO_TRUNCATION":
          return o("AdsVideoOptimizationVariants").AUTO_VIDEO_TRUNCATION;
        default:
          return null;
      }
    }
    function d(e) {
      if (e == null) return "OPT_OUT";
      switch (e) {
        case "OPT_IN":
          return "OPT_IN";
        default:
          return "OPT_OUT";
      }
    }
    function m(e) {
      if (e == null) return "NONE";
      switch (e) {
        case "OPTED_IN":
          return "OPTED_IN";
        case "DEFAULT_OPTED_IN_FIRST_TIME":
          return "DEFAULT_OPTED_IN_FIRST_TIME";
        case "OPTED_OUT":
          return "OPTED_OUT";
        default:
          return "NONE";
      }
    }
    l.default = u;
  },
  98,
);
