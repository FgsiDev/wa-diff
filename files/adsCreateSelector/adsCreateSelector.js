__d(
  "adsCreateSelector",
  [
    "AdsSelectorDebug",
    "AdsSelectorInstrumentation",
    "AdsSelectorNameUtils",
    "AdsSelectorUtils",
    "LRUKeyedCache",
    "MultiKeyMap",
    "adsCacheSelector",
    "adsMemoizeSelectorGetStores",
    "adsSelectorDisposal",
    "getErrorSafe",
    "makeStringifier",
    "qex",
    "shallowArrayEqual",
  ],
  function (t, n, r, o, a, i, l) {
    "use strict";
    var e, s, u;
    function c(e, t, n) {
      return e && n(e[0], t) ? e : [t];
    }
    function d(e, t, n) {}
    function m(e, t, n, r, o) {}
    function p(e, t, n) {}
    function _(e, t) {}
    function f(e, t) {}
    function g(e) {}
    function h(e) {}
    function y(e, t, n, o) {
      var a;
      return t
        ? function (n) {
            return (a || (a = e(n)), a);
          }
        : r("adsMemoizeSelectorGetStores")(n, e, o);
    }
    function C(e, t) {
      return (
        e === t ||
        (Array.isArray(e) && Array.isArray(t) && r("shallowArrayEqual")(e, t))
      );
    }
    function b(e) {
      return e
        ? function (t, n) {
            return t === n || e(t, n);
          }
        : C;
    }
    function v(e) {
      return typeof e.getStores == "function" ? e : null;
    }
    function S(t, n, a, i, l, s, u, c, d) {
      d === void 0 && (d = !1);
      var m = { cacheHit: !0 },
        p = t.bind(void 0, m);
      (o("AdsSelectorNameUtils").nameSelector(p, s + "-instrumentedSelectFn"),
        o("AdsSelectorNameUtils").nameSelector(t, s + "-selectFn"));
      var _, f;
      if (d) _ = p;
      else {
        var g = r("adsCacheSelector")(p, a, i, c, s);
        ((_ = g), (f = g.dispose));
      }
      var h = Object.assign(
        function (t) {
          m.cacheHit = !0;
          var n = (e || (e = o("AdsSelectorInstrumentation"))).onSelectorCall(),
            r = _(t);
          return (n && n({ name: s, cacheHit: m.cacheHit }), r);
        },
        {
          getStores: y(a, i, l, s),
          equal: u,
          isGetStoresStatic: i,
          isGetStoresPure: l,
          rawSelectFn: n,
        },
      );
      return (
        f != null && o("adsSelectorDisposal").registerAdsSelectorDisposer(h, f),
        o("AdsSelectorNameUtils").nameSelector(h, s),
        h
      );
    }
    function R(e, t, n, r, a) {
      var i = null;
      return S(
        function (n, o) {
          n.cacheHit = !1;
          var a = t.apply(
            void 0,
            e.map(function (e) {
              return e(o);
            }),
          );
          return ((i = c(i, a, r)), i[0]);
        },
        t,
        function (n) {
          return (s || (s = o("AdsSelectorUtils"))).getStores(
            [].concat(e, [t]),
            n,
          );
        },
        e.every(function (e) {
          return e.isGetStoresStatic;
        }) && t.isGetStoresStatic,
        e.every(function (e) {
          return e.isGetStoresPure && t.isGetStoresPure;
        }),
        n,
        r,
        a,
      );
    }
    function L(e, t, n, r) {
      e.set(t, { lastInputsForProps: n, lastResultForProps: r });
    }
    var E = Symbol(),
      k = 1e3;
    function I(e, t, n, a, i) {
      var l;
      if (r("qex")._("5867") === !0) {
        var u = new (r("MultiKeyMap"))(),
          y = E;
        l = function (r, o) {
          for (var n = new Array(e.length), i = 0; i < e.length; ++i)
            n[i] = e[i](o);
          var l = u.get(n);
          return (
            l === void 0 &&
              ((r.cacheHit = !1),
              (l = t.apply(void 0, n)),
              y !== E && a(l, y) && (l = y),
              u.set(n, l === void 0 ? E : l)),
            (y = l),
            l === E ? void 0 : l
          );
        };
      } else {
        var C = new (r("LRUKeyedCache"))(
            i != null
              ? i
              : function (e) {
                  return e;
                },
            k,
          ),
          b = null,
          v = null;
        l = function (o, i) {
          var n = [];
          n.length = e.length;
          for (var l = !1, s = 0; s < e.length; s++)
            ((n[s] = e[s](i)), (!v || v[s] !== n[s]) && (l = !0));
          var u = C.get(i);
          if (u == null);
          else {
            var y = u.lastInputsForProps,
              S = u.lastResultForProps;
            if (r("shallowArrayEqual")(y, n)) return ((v = y), (b = [S]), b[0]);
          }
          if (u == null && !l && v != null && b != null) {
            var R = v,
              E = b;
            return (L(C, i, R, E[0]), E[0]);
          }
          o.cacheHit = !1;
          try {
            var k = t.apply(void 0, n);
            (u != null && (b = [u.lastResultForProps]), (b = c(b, k, a)));
            var I = b;
            return ((v = n), L(C, i, v, b[0]), I[0]);
          } catch (e) {
            throw (r("getErrorSafe")(e), e);
          }
        };
      }
      return S(
        l,
        t,
        function (t) {
          return (s || (s = o("AdsSelectorUtils"))).getStores(e, t);
        },
        e.every(function (e) {
          return e != null && e.isGetStoresStatic;
        }),
        e.every(function (e) {
          return e != null && e.isGetStoresPure;
        }),
        n,
        a,
        i,
        r("qex")._("2315") === !0,
      );
    }
    function T(e, t, n, r) {
      var o = null,
        a = [];
      return S(
        function (t) {
          t.cacheHit = !1;
          var r = e();
          return ((o = c(o, r, n)), o[0]);
        },
        e,
        function () {
          return a;
        },
        !0,
        !0,
        t,
        n,
        r,
      );
    }
    function D(e, t, n) {
      var a, i;
      n === void 0 && (n = {});
      var l =
          (a = (i = n.name) != null ? i : t.name) != null
            ? a
            : "unnamedSelector",
        s = n.hashProps,
        u = s != null ? r("makeStringifier")(s) : void 0,
        c = b(n.equal),
        d = v(t),
        m;
      return (
        d
          ? (m = R(e, d, l, c, u))
          : e.length
            ? (m = I(e, t, l, c, u))
            : (m = T(t, l, c, u)),
        o("AdsSelectorNameUtils").nameSelector(m, l + "-wrappedSelector"),
        m
      );
    }
    l.default = D;
  },
  98,
);
