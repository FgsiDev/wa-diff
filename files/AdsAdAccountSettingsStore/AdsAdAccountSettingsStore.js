__d(
  "AdsAdAccountSettingsStore",
  [
    "AdsAdAccountSettingsStoreSetAPlusCFeatureStickyStatusMutation.graphql",
    "AdsAdAccountSettingsStoreSetVideoOptimizationStickyStatusMutation.graphql",
    "AdsAdAccountSettingsStoreSource",
    "AdsManagerRelayEnvironment",
    "RelayModern",
    "ReverseInteropStoreBase",
    "justknobx",
  ],
  function (t, n, r, o, a, i, l) {
    "use strict";
    var e,
      s,
      u = {
        aplusc_sticky_config: null,
        media_optimization_sticky_config: null,
      },
      c = "AdsAdAccountSettingsStore.DATA_UPDATED.",
      d =
        e !== void 0
          ? e
          : (e = n(
              "AdsAdAccountSettingsStoreSetAPlusCFeatureStickyStatusMutation.graphql",
            )),
      m =
        s !== void 0
          ? s
          : (s = n(
              "AdsAdAccountSettingsStoreSetVideoOptimizationStickyStatusMutation.graphql",
            )),
      p = (function (e) {
        function t() {
          var t = new (r("AdsAdAccountSettingsStoreSource"))();
          return e.call(this, t, c) || this;
        }
        babelHelpers.inheritsLoose(t, e);
        var n = t.prototype;
        return (
          (n.updateFeatureStickyStatus = function (t, n, a) {
            var e = this;
            r("justknobx")._("4395") &&
              o("RelayModern").commitMutation(r("AdsManagerRelayEnvironment"), {
                mutation: d,
                variables: {
                  input: { ad_account: t, feature_name: n, status: a },
                },
                onCompleted: function () {
                  e.refetchKeys([t]);
                },
              });
          }),
          (n.updateVideoOptimizationStickyStatus = function (t, n, a) {
            var e = this,
              i =
                n === "ai_video_shortening"
                  ? "AI_VIDEO_SHORTENING"
                  : n === "auto_video_truncation"
                    ? "AUTO_VIDEO_TRUNCATION"
                    : (function () {
                        throw Error(
                          "Match: No case succesfully matched. Make exhaustive or add a wildcard case using '_'. Argument: " +
                            n,
                        );
                      })();
            o("RelayModern").commitMutation(r("AdsManagerRelayEnvironment"), {
              mutation: m,
              variables: {
                input: { ad_account: t, optimization: i, status: a },
              },
              onCompleted: function () {
                e.refetchKeys([t]);
              },
            });
          }),
          (n.get = function (t) {
            return this.getSingleValue(t, u);
          }),
          t
        );
      })(r("ReverseInteropStoreBase"));
    p.__moduleID = i.id;
    var _ = new p(),
      f = _;
    l.default = f;
  },
  98,
);
